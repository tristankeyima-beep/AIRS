...
将 ADP 知识库检索结果中的 DOC 正文转换为可被现有审核流程消费的 `ruleRepository`。本节点链路只输出规则库；`logicTopology` 与患者材料审核不在本次范围内。
...
输出 `Output.ruleRepository`。LLM 负责读 DOC、拆准入条件、关联提取项细则，并输出：
...
LLM 不生成 `ruleCode` 与 `keywordCode`，避免模型造成编码重复、跳号或格式不一致。
...
代码负责拆开腾讯 `Output` 外层、校验 LLM 输出、补默认字段、分配确定性编码并排序。输出为 `ruleRepository`。

规则编码取 `chronicDiseaseCode` 的末两位数字并拼接三位序号：`M07801` 的第一条规则为 `01001`。每条规则的第一个提取项编码为 `01001001`，后续提取项按三位序号递增。病种编码不以两位数字结尾、规则数或单规则提取项数超过 999、规则或提取项字段不合规则时必须抛出错误，阻止不完整规则库进入下游。
...
验证以 `output.json` 中尿毒症透析 DOC 为例：规则库必须输出三条规则、编码依次为 `01001`、`01002`、`01003`；每个规则至少有一个提取项；所有枚举提取项有非空 `enumOptions`；JSON 文本与腾讯结构化对象都可被代码节点解析。
