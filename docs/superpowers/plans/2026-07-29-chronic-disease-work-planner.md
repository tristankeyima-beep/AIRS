...
---
name: chronic-disease-work-planner
description: 当用户明确要求制定计划、拆解任务或安排步骤，或任务目标模糊、资料较多，或涉及门诊慢特病六项能力中的两项及以上（含两项），或暂时无法判断应使用哪项能力时使用；非门诊慢特病任务不使用。
---
...
4. 目标模糊，或涉及门诊慢特病六项能力中的两项及以上（含两项）时，完整阅读：
   - `references/intent-routing.md`
   - `references/continuous-execution.md`
   - `references/markdown-plan-template.md`
5. 说明已识别内容、可完成的两至三个方向及各自成果。
6. 每轮集中询问一至三个会改变路线的问题，不重复询问已有信息。
7. 展示完整工作计划，并请用户选择只制定计划或自动连续执行；选择模式不等于授权执行。
8. 选择只制定计划时，更新执行方式和当前状态为“计划已制定”后停止，不调用下游能力；选择自动连续执行时，必须再明确确认按当前计划开始，才可执行。
9. 暂停时只在计划中记录问题摘要，详细澄清问题不得写入工作计划，必须在计划之外单独提问。
10. 完成后先列出“本次交付”及实际超链接，再展示最终计划状态。
...
只有已确认标准才能用于生成通用准备清单、材料预检、结构化或其他依赖正式标准的后续任务。

政策文件可以成为标准来源；临床指南、专家共识和诊疗规范只能作为讨论与修订参考，不得自动成为医保准入条件。

## 标准修改

用户希望修改认定标准时，按以下顺序执行：提供现行标准 → 确认拟修改内容、适用范围和业务原因或目标 → 检索政策、指南、共识等修改依据 → 生成拟修订版并取得用户确认 → 调用版本比对能力分析新增、删除、修改、逻辑变化和受影响审核规则 → 可选患者材料双版本判读并中立比较结果。
```
