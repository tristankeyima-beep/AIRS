...
首次先运行权威 `python3 scripts/inspect_standard.py <标准文件> --profile qc` 分类标准并记录缺陷/警告；该结果先于 inventory 的构造、展示与摘要，且 `inventory.standardKind` 必须与其一致。随后展示已收到的清单：患者材料（文件名/材料 ID/页或段）、认定标准、审核过程或明细、最终审核结论。清单中必须说明输入变体：
...
模式 1 正式交付与模式 2 外部输入使用不同口径。模式 1 的正式标准仍由 `validate_certification.py` 执行五位编码和正式字段契约；模式 2 先运行 `python3 scripts/inspect_standard.py <标准文件> --profile qc`，检查标准是否足以执行本次质控。

QC 兼容口径不限定编码必须四位或五位：同一标准内规则编码是非空字符串、全文唯一、不混用编码方案，逻辑引用全部可解析，即可继续。四位编码本身不是结构缺陷；缺少模式 1 专属来源字段或存在外部扩展字段只记录兼容性提示。重复编码、四位与五位混用、字母数字与纯数字混用、悬空引用、规则正文缺失或逻辑不可解析，才属于影响执行的结构缺陷。
...
患者材料正文进入规范对象前统一放到 `rawInput.materials[].content`。外部常见的 `materialContent`、`text`、`rawText` 由 `prepare_qc_report.py` 确定性归一；同一材料同时出现内容不同的正文键时 fail-closed，不猜测采用哪一个。
