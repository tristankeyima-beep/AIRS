...
def _evidence(items, path):
    ...
    for index, item in enumerate(items):
        ...
        _object(item["location"], f"{point}.location", {"start", "end"})
...
def _validate_report(report):
    ...
    for index, issue in enumerate(report["issues"]):
        ...
        _enum(issue["severity"], f"{point}.severity", SEVERITIES); _enum(issue["confidence"], f"{point}.confidence", CONFIDENCES); _enum(issue["impactOnFinalResult"], f"{point}.impactOnFinalResult", IMPACTS); _enum(issue["riskDirection"], f"{point}.riskDirection", ISSUE_RISKS); _enum(issue["evidenceStatus"], f"{point}.evidenceStatus", EVIDENCE_STATES); _evidence(issue["materialEvidence"], f"{point}.materialEvidence")
    ...
    for index, review in enumerate(report["ruleReviews"]):
        ...
        _enum(review["result"], f"{point}.result", RULE_RESULTS); _enum(review["evidenceStatus"], f"{point}.evidenceStatus", EVIDENCE_STATES); _evidence(review["materialEvidence"], f"{point}.materialEvidence")
...
def _evidence_text(evidence):
    ...
    for item in evidence:
        entries.append(f"材料 {item['materialId']}／{item['materialName']}；页：{item['page']}；章节：{item['section']}；定位：start={item['location']['start']}，end={item['location']['end']}；原文：{item['rawText']}；规范化：{item['normalizedText']}")
    return "\n".join(entries)


def _issue_text(issue):
    return f"- [{issue['severity']}] {issue['issueType']}（规则 {issue['ruleCode']}，关键词 {issue['keywordCode']}；证据状态 {issue['evidenceStatus']}；{RISK_LABELS[issue['riskDirection']]}；{IMPACT_LABELS[issue['impactOnFinalResult']]}；置信度 {issue['confidence']}）\n  模型主张：{issue['modelClaim']}\n  材料/标准证据：{_evidence_text(issue['materialEvidence'])}\n  质控发现：{issue['qcFinding']}\n  可能影响：{issue['possibleImpact']}\n  建议：{issue['recommendation']}"
...
def render_qc_text(source):
    ...
    lines = ["# 质控结论", f"结论：{report['qcConclusion']}", f"风险方向：{report['riskDirection']}", f"原审核结论：{report['originalResult']}", f"问题数量：{len(report['issues'])}", "", "# 输入与检查范围", f"案例：{report['case']['patientName']}／{report['case']['diseaseName']}／{report['case']['auditId']}", f"材料：{'、'.join(report['inputScope']['materials']) or '无'}", f"标准格式：{report['inputScope']['standardKind']}", f"审核结果类型：{report['inputScope']['auditResultKind']}"]
    ...
    if report["ruleReviews"]:
        for review in report["ruleReviews"]:
            lines += [f"- 规则 {review['ruleCode']}：{review['result']}；证据状态：{review['evidenceStatus']}", f"  模型主张：{review['modelClaim']}", f"  材料/标准证据：{_evidence_text(review['materialEvidence'])}", f"  质控发现：{review['qcFinding']}", f"  建议：{review['recommendation']}"]
    ...
    lines += ["", "# 原始输入", json.dumps(report["rawInput"], ensure_ascii=False, indent=2, sort_keys=True, allow_nan=False)]
    return "\n".join(lines) + "\n"
...
def _evidence_html(evidence):
    ...
    cards = []
    for item in evidence:
        cards.append('<div class="evidence"><div class="grid">' + ''.join(f'<div class="field"><b>{esc(label)}</b>{esc(value)}</div>' for label, value in (("材料编号", item["materialId"]), ("材料名称", item["materialName"]), ("页码", item["page"]), ("章节", item["section"]), ("定位起点", item["location"]["start"]), ("定位终点", item["location"]["end"]))) + '</div><p class="label">原始文本</p><blockquote>' + esc(item["rawText"]) + '</blockquote><p class="label">规范化文本</p><blockquote>' + esc(item["normalizedText"]) + '</blockquote></div>')
    return ''.join(cards)
