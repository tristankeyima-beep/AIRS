...
- found=true 但 value 表示不满足、否定或无法达标时，必须按“不通过”处理，并把 results[].rawText 写入 suspicionList.sources[].refContent；detail 要描述“已有相关证据但未达到规则要求”，不要写成“未见任何证据”。
- 对 enumOptions 包含“是/否/无法判断”的字段：value=“是”通常表示该提取项满足；value=“否”或“无法判断”通常表示该提取项不满足或证据不足。
- 对医院等级字段：value=“二级及以上”表示满足；value=“二级以下”或“无法判断”表示不满足或证据不足。
