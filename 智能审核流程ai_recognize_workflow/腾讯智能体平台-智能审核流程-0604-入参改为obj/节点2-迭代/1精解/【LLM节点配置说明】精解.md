...
| `material_list` | 引用 | [obj] | `开始.material_list` |
...
material_list: Array<Object>
  患者材料列表数组。0604 版本建议在腾讯平台配成 [obj]，避免 str 长度限制。
```
...
4. 找到明确否定、排除、未见、否认、未诊断等反向证据时，仍视为已找到相关证据：
   ...
   - results 正常返回
   - value 取最匹配的枚举值
5. 如果材料中出现与提取项相关但不满足认定条件的证据，不得输出 found=false；应输出 found=true，并在 results 中返回原文证据，value 取 enumOptions 中表示不满足或无法达标的选项，例如“否”“未确诊”“无法判断”等。
   - 例：提取项要求判断是否达到尿毒症期，材料仅记载“慢性肾脏病3期”“肌酐96”“估算肾小球滤过率50.29”，这是相关但不达标证据，应输出 found=true，value 取“否”或“无法判断”，rawText 返回对应原文。
   - 例：提取项要求判断是否长期透析，材料中没有透析记录但出现肾病诊断和治疗经过，应对长期透析项输出 found=false；如果材料明确写“未透析”“未行透析”，则输出 found=true，value 取“否”。
6. 只有完全找不到与该提取项相关的任何证据时，才返回：
   - found=false
   - results=[]
